...
class PanNukeDataset_mod(CellDataset):
    """PanNuke dataset
    ...
    def __init__(
        ...
        for fold in folds:
            ...
            ##fix: exclude for instance segmentation task, since type information is unknown
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, dict, str, str]:
        ...
        ##fix: tissue_type, type_map (dummy), to do: may exclude elements in the output dict()
        ...
        ##fix: need to fix, here should match the config.
        #tissue_type = 'unknown'
        tissue_type = 'Adrenal_gland'
        inst_map = mask.copy()
        np_map = mask.copy()
        np_map[np_map > 0] = 1
        type_map = np_map.copy()
        hv_map = PanNukeDataset_mod.gen_instance_hv_map(inst_map)
        ...
        masks = {
            ...
            "nuclei_type_map": torch.Tensor(type_map).type(torch.int64),    ##fix: set equal to np_map
        ...
        if self.stardist:
            dist_map = PanNukeDataset_mod.gen_distance_prob_maps(inst_map)
            stardist_map = PanNukeDataset_mod.gen_stardist_maps(inst_map)
            masks["dist_map"] = torch.Tensor(dist_map).type(torch.float32)
            masks["stardist_map"] = torch.Tensor(stardist_map).type(torch.float32)
        if self.regression:
            masks["regression_map"] = PanNukeDataset_mod.gen_regression_map(inst_map)
    ...
    def load_maskfile(self, index: int) -> np.ndarray:
        ...
        ##fix: new data label.npy does not contain 'dict' object
        inst_map = mask.astype(np.int32)

        # inst_map = mask[()]["inst_map"].astype(np.int32)  #default
        # type_map = mask[()]["type_map"].astype(np.int32)  #default
        # mask = np.stack([inst_map, type_map], axis=-1)    #default
        # return mask
        return inst_map
